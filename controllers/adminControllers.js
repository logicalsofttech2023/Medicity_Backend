const { packagecategoryModel,
    ...
    const updatePackageCategory=async(req,res)=>{
        try {
            ...
            if(!categoryId){
                return res.status(400).json({message:"categoryId name,image are  required"});
            }
            const packageCategory=await packagecategoryModel.findById(categoryId);
            if(!packageCategory){
                return res.status(404).json({message:"Package category not found"});
            ...
            await packagecategoryModel.findByIdAndUpdate({_id:categoryId},obj,{new:true});
    ...
    const addPackage=async(req,res)=>{
        try {
            const {title,actual_price,price,discount_price,
                ...
                interoduction,packageType,description,total_test,package_categoryId,test,offer,
            }=req.body;
            if(!gender || ! interoduction){
                return res.status(400).json({
                   message:'title,actual_price,price,discount_price,report_time,fasting_time,gender,ageGroup, interoduction,total_test,package_categoryId'});
            }
        ...
        // Convert `test` safely
        const tests = test ? test.split(',').map(item => item.trim()) : [];
         let image='';
         if(req.file){
        ...
        if (req.body.offer) {
            if (typeof req.body.offer === 'string') {
                try {
                    offerArray = JSON.parse(req.body.offer);
                } catch (error) {
                    return res.status(400).json({ message: "Invalid JSON format for offer" });
                }
            } else if (Array.isArray(req.body.offer)) {
                offerArray = req.body.offer;
            } else {
                return res.status(400).json({ message: "Offer must be an array" });
            }
        }
        ...
        // Convert `test` safely
        const tests = req.body.test ? req.body.test.split(',').map(item => item.trim()) : [];
                    ...
                    await packageModel.findByIdAndUpdate({_id:packageId},obj,{new:true});
                    ...
                    const deletePackage=async(req,res)=>{
                        try {
                            ...
                            res.status(200).json({result:true,message:"Package deleted successfully"});
                        } catch (error) {
                            res.status(500).json({ error: error.message });
                        }
                    };
                    ...
                    const adminLogin=async(req,res)=>{
                        try {
                            ...
                            res.status(200).json({result:true,message:"Admin login successfully",data:admin});
                        } catch (error) {
                            res.status(500).json({ error: error.message });
                        }
                    };
                    ...
                    const addCheckupRoutine=async(req,res)=>{
                        try {
                            ...
                            res.status(200).json({result:true,message:"Checkup routine added successfully"});
                        } catch (error) {
                            res.status(500).json({ error: error.message });
                        }
                    };
                    ...
                    const getSingleCheckupRoutine=async(req,res)=>{
                        try {
                            ...
                            const checkupRoutine=await checkupRoutineModel.findById({_id:checkupRoutineId});
                            if(!checkupRoutine){
                    ...
                    const updateCheckupRoutine=async(req,res)=>{
                        try {
                            ...
                            await checkupRoutineModel.findByIdAndUpdate({_id:checkupRoutineId},obj,{new:true});
                            res.status(200).json({result:true,message:"Checkup routine updated successfully"});
                        } catch (error) {
                            res.status(500).json({ error: error.message });
                        }
                    };
                    ...
                    const deleteCheckupRoutine=async(req,res)=>{
                        try {
                            ...
                            res.status(200).json({result:true,message:"Checkup routine deleted successfully"});
                        } catch (error) {
                            res.status(500).json({ error: error.message });
                        }
                    };
...
const userList=async(req,res)=>{
    try {
        ...
        res.status(200).json({result:true,message:"Users list got successfully",data:users});
        } catch (error) {
            res.status(500).json({ error: error.message });
            }
            };
                    ...
                    const faqList=async(req,res)=>{
                        try {
                            ...
                            if(faqs.length ===0){
                                ...
                                res.status(200).json({result:true,message:"Faq list got successfully",data:
                                    faqs});
                                    } catch (error) {
                                        res.status(500).json({ error: error.message });
                                        }
                                        ...
                                        const deleteFaq=async(req,res)=>{
                                            try {
                                                ...
                                                if(!faqId){
                                                    return res.status(404).json({result:false,message:"faqId is required"});
                                                    }
                        ...
                        const getFaq=async(req,res)=>{
                            try {
                                ...
                                if(!faq){
                                    ...
                                    res.status(200).json({result:true,message:"Faq got successfully",data:faq
                                        });
                                        } catch (error) {
                                            res.status(500).json({ error: error.message });
                                            }
                                            ...
                                            const updateFaq=async(req,res)=>{
                                                try {
                                                    const {faqId}=req.body;
                                                    const {question,answer}=req.body;
                                                    const faq=await faqModel.findByIdAndUpdate({_id:faqId},{question,answer},{
                                                        new:true
                                                        });
                                                        if(!faq){
                                                            return res.status(404).json({result:false,message:"Faq not found"});
                                                            }
                                                            res.status(200).json({result:true,message:"Faq updated successfully",data:faq
                                                                });
                                                                } catch (error) {
                                                                    res.status(500).json({ error: error.message });
                                                                    }
  ...
  const addAboutus=async(req,res)=>{
    try {
       ...
       if(!title || !description){
        return res.status(400).json({result:false,message:"Please fill all the fields  title,description"});                                                                                                           
       } 
       
       const aboutus=new  aboutUsModel({title,description});
       const result=await aboutus.save();
       res.status(200).json({result:true,message:"About us added successfully",data:result});
    ...
    const getAllAboutus=async(req,res)=>{
        try {
            ...
            if(!aboutus){
                return res.status(404).json({result:false,message:"About us not found"});
                }
                res.status(200).json({result:true,message:"About us list",data:aboutus
                    });
                    } catch (error) {
                        res.status(500).json({ error: error.message });
                        }
                        ...
                        const updateAboutus=async(req,res)=>{
                            try {
                                ...
                                if(!aboutusId){
                                    return res.status(400).json({result:false,message:"Please fill aboutusId,title,description"});
                                }
                                await aboutUsModel.findByIdAndUpdate({_id:aboutusId},{title,description},{new
                                    :true});
                                
                                 res.status(200).json({result:true,message:"About us updated successfully"});
 ...
 const getAboutusById=async(req,res)=>{
    try {
        ...
        if(!aboutus){
            ...
            res.status(200).json({result:true,message:"About us data",data:aboutus
                });
                } catch (error) {
                    res.status(500).json({ error: error.message });
                    }
                    ...
                    const deleteAboutus=async(req,res)=>{
                        try {
                            ...
                            if(!aboutus){
                                ...
                                res.status(200).json({result:true,message:"About us deleted successfully"});
                                } catch (error) {
                                    res.status(500).json({ error: error.message });
                                    }
                                };
  ...
  const addContactus=async(req,res)=>{
    try {
        ...
        if(!email||!phone||!whatsapp){
            return res.status(400).json({result:false,message:"Please fill name,email,phone,whatsapp"});
                }

                const exists=await contactUsModel.findOne();
                if(exists){
                    await contactUsModel.findOneAndUpdate({},{whatsapp,email,phone,name},{new:true});
                    return res.status(200).json({result:true,message:"Contact us updated successfully"});
                    } 
                    ...
                    const getContactus=async(req,res)=>{
                        try {
                            ...
                            if(!contactus){
                                return res.status(404).json({result:false,message:"Contact us not found"});
                                }
                                res.status(200).json({result:true,message:"Contact us data",data:contactus
                                    });
                                    } catch (error) {
                                        res.status(500).json({ error: error.message });
                                        }
...
const getAllOrders = async (req, res) => {
    ...
    if (!orders || orders.length === 0) {
        return res.status(200).json({ result: true, message: "All orders data", data: orders });
        } else {
        res.status(200).json({ result: true, message: "All orders data", data: orders });
    }
}catch(err){
    res.status(500).json({ result: false, message: "Something went wrong", error: err.message });
}
};
...
const getAllApointments = async (req, res) => {
    try{
    const orders = await appointmentModel.find() .populate({
        path: 'userId',
        model: 'user',
      })
...
//prescription file list
  const allPrescriptionFiles = async (req, res) => {
    ...
    const prescriptions = await prescriptionModel.find({}).populate('userId memberId');  
...
const addReports=async(req,res)=>{
    try{
        ...
        if(!userId){
            return res.status(400).json({result:false,message:"userId,testName,recordFor,comment are required"})
        }
        const obj={
            userId,
            testName,
...
const reportsList=async(req,res)=>{
    try{
        const data=await reportsModel.find().populate('userId').sort({createdAt: -1});
...
const getByIdApointments = async (req, res) => {
    try{
        ...
        if(!appointmentId){
            return res.status(400).json({result:false,message:"appointmentId is required"})
        }
    const orders = await appointmentModel.findById({_id:appointmentId}) .populate({
        path: 'userId',
        model: 'user',
      })
...
const getByIdOrders = async (req, res) => {
    try{
        ...
        if(!bookingId){
            return res.status(400).json({result:false,message:"bookingId is required"})
        }
    const orders = await bookingOrderModel.findById({_id:bookingId}) .populate({
        path: 'userId',
        model: 'user',
      })
      .populate({
...
const createBlogCategory=async(req,res)=>{
    try{
        ...
        if(!name ){
            ...
            if(existData){
                await blogCategoryModel.findOneAndUpdate({name:name},{$set:{name:name}},{new:true});
               return res.status(200).json({result:true,message:"Updated successfully"})
                }
            ...
            const blogCategory=new blogCategoryModel({
                ...
                res.status(200).json({result:true,message:"Blog Category created successfully"});
                }catch(err){
    ...
    const blogCategoryList=async(req,res)=>{
        try{
            ...
            res.status(200).json({result:true,message:"Blog Category list",data:blogCategories
                });
                }catch(err){
                    res.status(500).json({result:false,message:err.message});
                    }
    ...
    const deleteBlogCategory=async(req,res)=>{
        try{
            ...
            if(!blogCategory){
                return res.status(400).json({result:false,message:"Blog Category not found"});
                }
                res.status(200).json({result:true,message:"Blog Category deleted successfully"});
                }catch(err){
                    res.status(500).json({result:false,message:err.message});
                    }
    ...
    const createBlogPost=async(req,res)=>
        {
            try{
                ...
                const blogPost=new blogModel({
                    ...
                    await blogPost.save();
                    res.status(200).json({result:true,message:"Blog Post created successfully"});
                    }catch(err){
                        res.status(500).json({result:false,message:err.message});
                        }
                    };
...
const updateBlog=async(req,res)=>
    {
        try{
            ...
            if(!blogId ){
                return res.status(400).json({result:false,message:"blogId and all fields are required"}
                    );
                    }
                    ...
                    if(!blog){
                        ...
                        await blog.save();
                        res.status(200).json({result:true,message:"Blog updated successfully"});
                        }catch(err){
                            res.status(500).json({result:false,message:err.message});
                            }
                            }
                            ...
                            const getBlog=async(req,res)=>
                                {
                                    try{
                                        ...
                                        if(!blogId ){
                                            return res.status(400).json({result:false,message:"blogId is required"}
                                                );
                                                }
                                                const blog=await blogModel.findById(blogId);    
                                                if(!blog){
                                                ...
                                                res.status(200).json({result:true,message:"Blog list is..",data:blog});
                                    }catch(err){
                                        res.status(500).json({result:false,message:err.message});
                                    }
                                };
        ...
        const bookingOrderList = async (req, res) => {
                    try {
                      ...
                      const bookings = await bookingOrderModel.find()
                        .populate({
                          path: 'userId',
                          model: 'user',
                        })
  ...
  const bookingOrderDetails = async (req, res) => {
                    try {
                      ...
                      if(!orderId){
                        return res.status(400).json({result:false,message:"orderId is required."});
                      }
                      const bookings = await bookingOrderModel.find({_id:orderId})
                        .populate({
                          path: 'userId',
                          model: 'user',
                        })
  ...
  const bookingOrderStatusChange = async (req, res) => {
                    try {
                      ...
                      if(!orderId || !status){
                        return res.status(400).json({result:false,message:"orderId ,status(1 for completed and 2 for cancelled) is required."});
                      }
                       await bookingOrderModel.findByIdAndUpdate({_id:orderId},{bookingStatus:status},{new:true})
